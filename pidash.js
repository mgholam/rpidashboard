...
function getip() {
    ...
    var addr = [];
    for (var k in interf){
        for (var k2 in interf[k]){
            var ad = interf[k][k2];
            if (ad.family === 'IPv4' && !ad.internal) {
                addr.push(ad.address);
            }
        }
    }
    return addr;
...
function sysinfo(){
    ...
    var uname = execSync('/bin/uname -a').toString();
    // return format for 'uptime':
